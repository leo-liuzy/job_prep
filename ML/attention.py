...
class SelfAttention(nn.Module):
    def __init__(self, model_dim, max_length, dropout=0.1):
        super().__init__()
        ...
        self.mask = torch.tril(torch.ones(max_length, max_length)).unsqueeze(0)
    ...
    def forward(self, x):
        ...
        scores = Q @ K.transpose(1, 2) / torch.sqrt(self.model_dim)
...
class MultiHeadAttentionKVCache(nn.Module):
    def __init__(self, model_size, max_length, n_head, dropout=0.1):
        ...
        self.n_head = n_head
        self.head_dim = model_size // n_head
        # mask: token at i can see tokens at non-zero entries of mask[i, :]
        self.register_buffer("mask", torch.tril(torch.ones(max_length, max_length)))
        self.mask = self.mask.unsqueeze(0).unsqueeze(0)
    ...
    def forward(self, x, kv_cache=None):
        ...
        # [B, n_head, T, T] or [B, n_head, T, seq_len]
        scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.head_dim**0.5)
        if kv_cache is not None:
...
class GroupedQueryAttention(nn.Module):
    def __init__(self, model_size, max_length, n_head, n_group=1, dropout=0.1):
        ...
        self.register_buffer("mask", torch.tril(torch.ones(max_length, max_length)))
        self.mask = self.mask.unsqueeze(0).unsqueeze(0).unsqueeze(0)
    ...
    def forward(self, x):
        ...
        # Group Q into (B, n_group, heads_per_group, T, head_dim)
        Q = Q.view(B, self.n_group, self.heads_per_group, T, self.head_dim)
        # Expand K, V for broadcasting over heads_per_group
        K = K.unsqueeze(2)  # (B, n_group, 1, T, head_dim)
        V = V.unsqueeze(2)  # (B, n_group, 1, T, head_dim)
        #### Difference ####

        # Attention scores: (B, n_group, heads_per_group, T, T)
        scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.head_dim**0.5)
        mask = self.mask[:, :, :, :T, :T].to(x.device)
        scores = scores.masked_fill(mask == 0, float("-inf"))  # (B, n_head, T, T)
        attn = F.softmax(scores, dim=-1)
