...
class QuantizedVerySimpleNet(nn.Module):
    ...
    def forward(self, img):
        x = img.view(-1, 28 * 28)
        x = self.quant(x)  # this is different from original model
        x = self.relu(self.linear1(x))
        x = self.relu(self.linear2(x))
        x = self.linear3(x)
        x = self.dequant(x)  # this is different from original model
        return x


test(net_quantized)  # run through calibration set
