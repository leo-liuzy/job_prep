...
class Transformer(nn.Module):
    def __init__(
        ...
        self.pos_encoding = PositionalEncoding(model_size, max_length)
        self.dropout = nn.Dropout(dropout)
    ...
    def encode(self, src, src_mask=None):
        ...
        src: (B, T_enc)
        """
        x = self.token_embedding(src) * math.sqrt(self.model_size)
    ...
    def decode(self, tgt, memory, src_mask=None, tgt_mask=None):
        """
        tgt: (B, T_dec)
        memory: (B, T_enc, D) encoder outputs
        """
        x = self.token_embedding(tgt) * math.sqrt(self.model_size)
        x = self.pos_encoding(x)
        x = self.dropout(x)
