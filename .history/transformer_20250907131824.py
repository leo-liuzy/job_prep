...
class MultiHeadAttention(nn.Module):
    def __init__(self, model_size, n_head, dropout=0.1):
        super().__init__()  # important!
    ...
    def forward(self, x):
        ...
        # [B, n_head, T, T]
        socres = torch.matmul(Q, K.transpose(-2, -1)) / (self.head_dim**0.5)


class TransformerLayer(nn.Module):
    def __init__(self, model_size, n_head, dropout):
        super().__init__()  # important!
        self.attn = MultiHeadAttention(n_head, head_size)
        self.ffn = FeedFoward(model_size, dropout)
